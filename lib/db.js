...
export async function obtenerPresupuestoPorId(id, agenteId, rol) {
    ...
    return {
        ...
        vendido: Boolean(presupuesto.vendido),
        fechaVenta: presupuesto.fecha_venta,
...
export async function marcarVendido(id, vendido, agenteId, rol) {
    const db = getDB();
    ...
    if (rol !== 'admin' && existing.rows[0].agente_id !== agenteId) {
        throw new Error('Sin permisos para modificar este presupuesto');
    }

    // Actualizar estado de venta
    const fechaVenta = vendido ? new Date().toISOString() : null;

    await db.execute(
        "UPDATE presupuestos SET vendido = ?, fecha_venta = ?, updated_at = datetime('now') WHERE id = ?",
        [vendido ? 1 : 0, fechaVenta, id]
    );
